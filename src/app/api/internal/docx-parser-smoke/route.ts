...
export async function GET() {
  ...
  const parsed = await parseDocx(Buffer.from(FIXTURE_BASE64, 'base64'));
  return NextResponse.json({
    hasMarker: parsed.fullText.includes('BLUE-SQUARE-991'),
    fullText: parsed.fullText,
    imageCount: parsed.embeddedImages.length,
    images: parsed.embeddedImages.map((image) => ({
      index: image.index,
