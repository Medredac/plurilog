      ...
      parameters: {
        ...
        properties: {
          ...
          reference_index: {
            ...
            minimum: 1,
            description:
              'Optional 1-based Image N selector for visual resources actually attached to this model call. Use it only when the user has clearly identified one of multiple visible images by content, position, filename, or another specific description. Do not choose an index for a vague request such as "edit the image".',
          },
        },
...
function formatImageBlockLabel(
  ...
  if (attachment.provenance === 'historical_assistant_generated') {
    const seatName = attachment.creatorSeatId
      ...
      : 'an assistant';
    return `${ordinalPrefix}originally generated by ${seatName} earlier in this discussion: ${cleanName}`;
  }
  if (attachment.provenance === 'same_round_assistant_generated') {
    const seatName = attachment.creatorSeatId
      ? (SEAT_DISPLAY_NAMES[attachment.creatorSeatId.toLowerCase()] || attachment.creatorSeatId)
      : 'an assistant';
    return `${ordinalPrefix}generated by ${seatName} earlier in the current round: ${cleanName}`;
  }
  if (attachment.provenance === 'historical_user_upload') {
    return `${ordinalPrefix}previously uploaded by the user earlier in this discussion: ${cleanName}`;
  }
  if (attachment.provenance === 'current_user_upload') {
    return `${ordinalPrefix}attached by the user in the current turn: ${cleanName}`;
  }
  return `${ordinalPrefix}file: ${cleanName}`;
}
