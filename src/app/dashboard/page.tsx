...
export default function DashboardPage() {
  ...
  const runRelay = async (
    ...
    try {
          ...
          try {
            ...
            if (eventType === 'seat_start') {
              ...
              const ownsGeneration =
                !discussionId || activeGen?.controller === controller;

              if (discussionId && ownsGeneration) {
                activeGenerationsRef.current.delete(discussionId);
              }
              if (abortControllerRef.current === controller) {
                abortControllerRef.current = null;
              }
              if (isCurrentDiscussionActive && ownsGeneration) {
                setActiveSpeaker(null);
                setIsDebating(false);
                setCanContinue(true);
              }
      ...
      if (activeDebateIdRef.current === discussionId && ownsGeneration) {
        setIsDebating(false);
        setActiveSpeaker(null);
      }
      refreshCreditStatus();
      ...
      try {
        for (let i = 0; i < preparedUploadBodies.length; i++) {
          ...
          const { data: signedData, error: signError } = await supabase.storage
            .from('message-images')
            .createSignedUrl(filePath, 259200); // 72h transport window; never the durable attachment identity
