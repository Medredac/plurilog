...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      localStorage.setItem('plurilog_auth', 'true');
      setIsLoading(false);
      onSuccess();
    }, 400);
  };
      ...
      <div className="relative w-full max-w-sm rounded-2xl bg-white border border-zinc-200/90 p-6 shadow-lg z-10 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex flex-col items-center text-center mb-5">
          ...
          <p className="text-xs text-zinc-500 mt-0.5">
            Access multi-model AI reasoning discussions.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-2 px-3 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-white font-medium text-xs shadow-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer mt-1"
          >
        ...
        <div className="relative my-4">
          ...
          <div className="relative flex justify-center text-[10px] text-zinc-400 uppercase tracking-wider bg-white px-2">
            or
          </div>
        </div>

        {/* Social / Instant Access Buttons */}
        <div className="space-y-2">
          <button
            type="button"
            onClick={handleGuestSignIn}
            className="w-full py-2 px-3 rounded-lg bg-white hover:bg-zinc-50 border border-zinc-200/80 text-zinc-700 font-medium text-xs transition-colors flex items-center justify-center gap-2 cursor-pointer shadow-2xs"
          >
          ...
          <button
            type="button"
            onClick={handleGuestSignIn}
            className="w-full py-1.5 px-3 rounded-lg bg-amber-50/70 hover:bg-amber-100/70 border border-amber-200/80 text-amber-950 font-medium text-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer shadow-2xs"
          >
