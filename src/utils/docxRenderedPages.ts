...
import type { RenderedDocxPage } from '@/utils/docxPageRenderer';

const STORAGE_BUCKET = 'message-images';
const MODEL_TRANSPORT_URL_EXPIRY_SECONDS = 259200;

export interface PersistedDocxRenderedPage {
  pageNumber: number;
  filename: string;
  storagePath: string;
  signedUrl: string;
...
export interface PersistDocxRenderedPagesOptions {
  supabase: any;
...
/**
 * Persists rendered DOCX pages as hidden derived visual assets.
 * Paths are deterministic from the parent DOCX hash + rendered page hash so
 * repeated visual inspection does not create duplicate storage objects.
 */
export async function persistDocxRenderedPages(
  options: PersistDocxRenderedPagesOptions
): Promise<PersistedDocxRenderedPage[]> {
  ...
  if (authError || !user) {
    throw new Error('Authenticated user session required for DOCX rendered-page persistence');
  }
  ...
  const persisted: PersistedDocxRenderedPage[] = [];
  ...
  for (const page of pages) {
    ...
    const storagePath =
      `${user.id}/docx-pages/${parentHash}/page-${pageNumber}-${pageHash.slice(0, 16)}.png`;
    ...
    if (signError || !signedData?.signedUrl) {
      console.warn('[DOCX Visual] Rendered page signing failed:', {
        parentFilename,
        pageNumber: page.pageNumber,
