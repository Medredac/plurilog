...
def patch_run_size(rxml, pxml, absolute=None, delta=None):
    ...
    size = current + float(delta) if delta is not None and abs(float(delta)) > 0.0001 else (float(absolute) if absolute is not None else current)
